Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/f5c8fb738726e644bb3c06fc0f513ff3"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalScale = float4(0.8,0.8,1,0);
				Decal0.m_DecalSideOffset = float4(0,-0.5,-0.5,0);
				Decal0.m_DecalPosition = float4(0.4,0.6,0,0);
				...
				{
					udonVideoAspectRatio = Decal0.GetVideoAspectRatio(_Udon_VideoTex_TexelSize.zw, (2.0 /*_Decal0VideoAspectFix*/), (1.0 /*_Decal0VideoFitToScale*/));
				...
				Decal1.m_DecalScale = float4(0.8,0.8,1,0);
				Decal1.m_DecalSideOffset = float4(0,-0.5,-0.5,0);
				Decal1.m_DecalPosition = float4(0.7,0.6,0,0);
				...
				{
					udonVideoAspectRatio = Decal1.GetVideoAspectRatio(_Udon_VideoTex_TexelSize.zw, (2.0 /*_Decal1VideoAspectFix*/), (1.0 /*_Decal1VideoFitToScale*/));
				...
				Decal2.m_DecalScale = float4(0.8,0.8,1,0);
				Decal2.m_DecalSideOffset = float4(0,-0.5,-0.5,0);
				Decal2.m_DecalPosition = float4(1.090705,0.5907326,0,0);
				...
				{
					udonVideoAspectRatio = Decal2.GetVideoAspectRatio(_Udon_VideoTex_TexelSize.zw, (2.0 /*_Decal2VideoAspectFix*/), (1.0 /*_Decal2VideoFitToScale*/));
					if ((0.0 /*_Decal2OnlyVideo*/))
					{
						if (udonVideoTexAvailable)
						{
							Decal2.m_DecalEmissionStrength += (0.0 /*_Decal2VideoEmissionStrength*/);
							if ((1.0 /*_Decal2UseDecalAlpha*/))
							{
								Decal2.SampleDecal(_DecalTexture2, poiMods, poiLight, poiMesh, poiCam);
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float4 flipbookScaleOffset = float4(0.2,0.2,0,0);
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
				...
				float theta = radians((0.0 /*_FlipbookRotation*/) + _Time.z * (100.0 /*_FlipbookRotationSpeed*/));
				float cs = cos(theta);
				float sn = sin(theta);
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, flipBookPixel.rgb, (0.0 /*_FlipbookBlendType*/)), flipBookPixel.a * float4(1,1,1,1).a * (1.0 /*_FlipbookReplace*/) * flipBookMask * flipbookAlpha);
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					float2 st = (poiMesh.uv[(0.0 /*_GlitterUV*/)] + float4(0,0,0,0).xy * _Time.x) * (10.0 /*_GlitterFrequency*/);
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						size = saturate(size + lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(2.0 /*_GlitterALSizeAddBand*/)]));
					}
					#endif
					half glitterAlpha = 1;
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						glitterMask = clamp(glitterMask + lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(2.0 /*_GlitterALAlphaAddBand*/)]), 0, glitterMask);
					}
					#endif
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						GlitterbrightnessOffset = max(GlitterbrightnessOffset +lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(2.0 /*_GlitterALMaxBrightnessBand*/)]), 0);
					}
					#endif
			...
			void ApplyPositionText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(-0.07,0.07,0,0), (0.0 /*_TextPositionRotation*/), float4(0.73,0.08,1,1), uv);
			...
			void ApplyTimeText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(-0.23,0,0,0), (0.0 /*_TextTimeRotation*/), float4(0.47,0.08,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(0,0.4034185,1,1).rgb, (0.0 /*_TextTimeColorThemeIndex*/)), opacity * float4(0,0.4034185,1,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(0,0.4034185,1,1).rgb, (0.0 /*_TextTimeColorThemeIndex*/)) * opacity * (0.0 /*_TextTimeEmissionStrength*/);
			}
			void ApplyFPSText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(-0.34,0.13,0,0), (0.0 /*_TextFPSRotation*/), float4(0.23,0.06,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
			...
			{
				if ((4.0 /*_TextNumericWholeDigits*/) == 0 && (4.0 /*_TextNumericDecimalDigits*/) == 0)
				{
					return;
				}
				uint wholeNumber = 0;
				...
				uint decimalDigits = (4.0 /*_TextNumericDecimalDigits*/);
				...
				float charSign = (1234.568 /*_TextNumericValue*/) >= 0 ? ASCII_SPACE : ASCII_NEGATIVE;
				NumericArray[arrayIndex] = charSign;						//First character is always the sign
				arrayIndex++;
				if (wholeDigits > 0)
				{
					wholeNumber = uint(glsl_mod(abs((1234.568 /*_TextNumericValue*/)), pow(10, wholeDigits)));
				...
				if (decimalDigits > 0)
				{
					...
					arrayIndex++;
					totalCharacters++;
					decimalNumber = uint(frac(abs((1234.568 /*_TextNumericValue*/))) * pow(10.00001, decimalDigits));    // Isolate the decimal number
				...
				uv = TransformUV(float4(0,-0.15,0,0), (0.0 /*_TextNumericRotation*/), float4(1,0.12,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
