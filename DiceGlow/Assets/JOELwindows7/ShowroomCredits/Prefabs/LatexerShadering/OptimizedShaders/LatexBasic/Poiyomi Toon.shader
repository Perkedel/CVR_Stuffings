Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/c750d82692983c241ac44c8604600e41"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 9.0.56</color>", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			void initPoiAudioLink(inout PoiMods poiMods)
			{
				...
				if (AudioLinkIsAvailable())
				{
					...
					poiMods.audioLink[0] = (0.027 /*_AudioLinkSmoothingBass*/) == 0 ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 0))[0] : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - (0.027 /*_AudioLinkSmoothingBass*/)) * 15.95, 0))[0];
					poiMods.audioLink[1] = (0.067 /*_AudioLinkSmoothingLowMid*/) == 0 ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 1))[0] : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - (0.067 /*_AudioLinkSmoothingLowMid*/)) * 15.95, 1))[0];
					poiMods.audioLink[2] = (0.105 /*_AudioLinkSmoothingHighMid*/) == 0 ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 2))[0] : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - (0.105 /*_AudioLinkSmoothingHighMid*/)) * 15.95, 2))[0];
					poiMods.audioLink[3] = (0.148 /*_AudioLinkSmoothingTreble*/) == 0 ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 3))[0] : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - (0.148 /*_AudioLinkSmoothingTreble*/)) * 15.95, 3))[0];
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				float theta = radians((15.5 /*_ALUVRotation*/) + _Time.z * (0.0 /*_ALUVRotationSpeed*/));
				float cs = cos(theta);
				float sn = sin(theta);
				uv = float2((uv.x - decalCenter.x) * cs - (uv.y - decalCenter.y) * sn + decalCenter.x, (uv.x - decalCenter.x) * sn + (uv.y - decalCenter.y) * cs + decalCenter.y);
			...
			{
				float clearCoatMask = (0.019 /*_ClearCoatStrength*/);
