Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/fff72cd276a9464448800a9284fcce7a"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.4);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.4);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.4) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.4));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.4));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.4) * poiLight.shadowMask);
