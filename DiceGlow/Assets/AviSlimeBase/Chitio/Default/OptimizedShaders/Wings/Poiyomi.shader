Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/97c70498cee67b1419d87d79b19d83e0"
{
	Properties
	{
 ...
 #define PROP_GLITTERMASK 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.7);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.7);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.7) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				poiLight.rampedLightMap = saturate((poiLight.lightMap - (1 - float(1))) / saturate((1 - float(0)) - (1 - float(1)) + fwidth(poiLight.lightMap)));
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.7));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.7));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.7) * poiLight.shadowMask);
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rim = 1 - smoothstep(min(float(0.044), rimWidth), rimWidth, viewDotNormal);
				...
				float3 finalRimColor = rimColor.rgb * lerp(1, poiFragData.baseColor, float(0.77));
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(200);
				...
				switch(float(0))
				{
					...
					if (float(2.5) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(90), float(2.5));
					}
					else
					{
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.85)), poiCam.viewDir)), float(100)), 0);
					finalGlitter *= glitterAlpha;
					break;
					case 1:
					float offset = random(randoPoint);
					float brightness = sin((_Time.x + offset) * float(2.5)) * float(20) - (float(20) - 1);
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(100)), 0);
