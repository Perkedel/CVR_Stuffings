Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/97c4d702fa65e46478da8e02795b1150"
{
	Properties
	{
 ...
 #define PROP_SET_HIGHCOLORMASK 
			...
			v2f vert(appdata v)
			{
				...
				if(float(0) == 0) // Discard Vertices instead of just pixels
				{
					...
					isDiscarded += (udim.y >= 1 && udim.y < 2) * dot(float4(float(0), float(0), float(0), float(0)), xMask);
					isDiscarded += (udim.y >= 2 && udim.y < 3) * dot(float4(float(0), float(0), float(0), float(0)), xMask);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(0))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.47) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.47) + 0.5;
						break;
					}
					case 2:
					{
				...
				blendMatcap(poiLight, poiFragData, float(0.2), float(0), float(0), float(0), float(1), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				#endif
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(0,1,0,0), float(0), float4(1,1,0,0), float(0), float(1));
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				#endif
				applyALEmmissionStrength(poiMods, emissionStrength1, float4(0,1,0,0), float(3), float4(1,1,0,0), float(0), float(1));
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.4)))) * float(1);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.4), 5)), float(0)) * float(1);
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.4)))) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.4), 5)), float(0)) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
