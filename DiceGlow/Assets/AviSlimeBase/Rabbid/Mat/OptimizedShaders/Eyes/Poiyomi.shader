Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/db24e8e26e413ff4e98e3e64494e30d5"
{
	Properties
	{
		...
		_EmissionStrength_Eyes ("Emission Strength", Range(0, 20)) = 0
		[Space(4)]
		...
		_EmissionStrength1_Eyes ("Emission Strength", Range(0, 20)) = 0
		[Space(4)]
 ...
 #define PROP_EMISSIONMAP1 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha_Eyes + audioLinkDecalAlpha0));
				if (float(0))
				{
					alphaOverride += 1;
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(1,0,0,0).rgb);
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(0))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), _ShadowStrength);
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = _EmissionStrength_Eyes;
				float3 emissionColor0 = 0;
				float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(0.8), float(1), float(0), float(1), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength1 = _EmissionStrength1_Eyes;
				float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(0.8), float(1), float(0), float(1), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP1) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float3 reflCol = GetReflections(poiCam, poiLight, poiMesh, roughness, float(0), float(1), _MochieReflCube, poiCam.reflectionDir);
			...
			void applyEnvironmentRim(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				float3 envRimCol = lerp(0, max(0, (enviroRimColor - float(0.1)) * poiFragData.baseColor), enviroRimAlpha).rgb * enviroMask * float(1);
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.3)))) * float(1);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.3), 5)), float(0)) * float(1);
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.3)))) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.3), 5)), float(0)) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.alpha = float(1) ? 1 : poiFragData.alpha;
