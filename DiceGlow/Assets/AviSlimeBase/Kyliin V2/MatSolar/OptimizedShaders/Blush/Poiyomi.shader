Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/90ec47558d4e44d44825b0961081a8c5"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_TEXTURERAMP 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.6);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.6);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.6) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(0))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.6));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.6));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.6) * poiLight.shadowMask);
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1490196,0,0.3176471,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1490196,0,0.3176471,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1490196,0,0.3176471,1).rgb, float(0));
				#endif
				if (float(0))
				{
