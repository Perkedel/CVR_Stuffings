Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/cd38558f58d8f324a8177a3875e93c10"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMASK2 
			...
			void calculateRGBNormals(inout PoiMesh poiMesh)
			{
				...
				{
					#if defined(PROP_RGBMASK) || !defined(OPTIMIZER_ENABLED)
					rgbMask = POI2D_SAMPLER_PAN(_RGBMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
					#else
					rgbMask = 1;
					#endif
				}
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 red = POI2D_SAMPLER_PAN(_RedTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float4 red = 1;
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 green = POI2D_SAMPLER_PAN(_GreenTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float4 green = 1;
				#endif
				#if defined(PROP_BLUETEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 blue = POI2D_SAMPLER_PAN(_BlueTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float4 blue = 1;
				#endif
				#if defined(PROP_ALPHATEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 alpha = POI2D_SAMPLER_PAN(_AlphaTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float4 alpha = 1;
				#endif
				float4 maskFinal = 1;
				maskFinal.r = saturate(rgbMask.r + float(0)) * saturate(red.a * _RedColor.a);
				maskFinal.g = saturate(rgbMask.g + float(0)) * saturate(green.a * float4(1,1,1,0).a);
				maskFinal.b = saturate(rgbMask.b + float(0)) * saturate(blue.a * float4(1,1,1,0).a);
				maskFinal.a = saturate(rgbMask.a + float(0)) * saturate(alpha.a * float4(1,1,1,0).a);
				if (float(0))
				{
					float3 RGBColor = 1;
					RGBColor = lerp(RGBColor, red.rgb * poiThemeColor(poiMods, _RedColor.rgb, float(0)), maskFinal.r);
					RGBColor = lerp(RGBColor, green.rgb * poiThemeColor(poiMods, float4(1,1,1,0).rgb, float(0)), maskFinal.g);
					RGBColor = lerp(RGBColor, blue.rgb * poiThemeColor(poiMods, float4(1,1,1,0).rgb, float(0)), maskFinal.b);
				...
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, red.rgb * poiThemeColor(poiMods, _RedColor.rgb, float(0)), maskFinal.r);
					poiFragData.baseColor = lerp(poiFragData.baseColor, green.rgb * poiThemeColor(poiMods, float4(1,1,1,0).rgb, float(0)), maskFinal.g);
					poiFragData.baseColor = lerp(poiFragData.baseColor, blue.rgb * poiThemeColor(poiMods, float4(1,1,1,0).rgb, float(0)), maskFinal.b);
