Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/2d6484995d354a6448598ab8cd33be1e"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_TEXTURERAMP 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1) == 1)
				{
					...
					#endif
					if (float(0.5) == float(0.25)) float(0.5) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0.5), float(0.25), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0.25), float(0.5), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0.25), float(0.5), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(1) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0.25), float(0.5), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
