Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/eb079e4ce42b14e4296d445bde181c91"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_TEXTURERAMP 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.45);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.45);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.45) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				#endif
				#ifdef _LIGHTINGMODE_TEXTURERAMP
				poiLight.rampedLightMap = lerp(1, UNITY_SAMPLE_TEX2D(_ToonRamp, poiLight.lightMap + float(0.25)).rgb, shadowStrength);
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(0))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.45));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.45));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.45) * poiLight.shadowMask);
				...
				if (float(1) == 1)
				{
					...
					#endif
					if (float(0.6) == float(0.4)) float(0.6) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0.6), float(0.4), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0.4), float(0.6), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0.4), float(0.6), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(1) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0.4), float(0.6), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(float(0), float(0.6)), float(0.6), poiCam.vDotN));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(1))
				{
					...
					if (float(1))
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, hueShift(poiFragData.baseColor, shift + float(1) * _Time.x), hueShiftAlpha.r);
					}
					else
					{
						poiFragData.baseColor = hueShift(poiFragData.baseColor, frac((shift - (1 - hueShiftAlpha.r) + float(1) * _Time.x)));
					}
				}
