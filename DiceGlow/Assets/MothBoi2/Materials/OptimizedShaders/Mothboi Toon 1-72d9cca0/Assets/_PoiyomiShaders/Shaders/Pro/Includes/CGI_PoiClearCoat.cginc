...
void calculateAndApplyClearCoat(inout float4 finalColor)
{
		...
		half clearCoatMap = POI2D_SAMPLER_PAN(_ClearcoatMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
	#else
		half clearCoatMap = 1;
	#endif
	...
	float NormalDotView = abs(dot(_Clearcoat, float(0) == 0 ? poiLight.N0DotV: poiLight.nDotV).r);
	#ifdef POI_LIGHTING
