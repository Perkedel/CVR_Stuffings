...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    if (float(0))
    {
        emissionStrength0 *= calculateBlinkingEmission(float(1), float(3), float(4), float(0));
    }
    
    emissionColor0 = hueShift(emissionColor0, frac(float(0) + _EmissionHueShiftSpeed * _Time.x) * float(0));
    ...
    float3 emissionColor1 = 0;
        ...
        emissionColor1 = hueShift(emissionColor1, frac(float(0) + _EmissionHueShiftSpeed1 * _Time.x) * float(0));
        #if defined(PROP_EMISSIONMASK1) || !defined(OPTIMIZER_ENABLED)
