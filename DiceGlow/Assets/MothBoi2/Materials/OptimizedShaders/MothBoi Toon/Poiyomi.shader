Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/14598788f8f50bc40a359ec054b6a1dd"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			{
				float MainColorFeatherStep = float(0.565) - float(0.435);
				...
				float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.565) - MainColorFeatherStep) * (shadowMask));
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / (float(0) - firstColorFeatherStep) * (shadowMask));
				mainShadowMask *= poiLight.shadowMask * float(0.654);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.654);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.654) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.654));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.654));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.654) * poiLight.shadowMask);
