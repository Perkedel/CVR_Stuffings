Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/9863a46e55f8904419d2fc1c4a975e7e"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0)
				{
					...
					if (1.0)
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, hueShift(poiFragData.baseColor, shift + 4.0 * _Time.x), hueShiftAlpha.r);
					}
					else
					{
						poiFragData.baseColor = hueShift(poiFragData.baseColor, frac((shift - (1 - hueShiftAlpha.r) + 4.0 * _Time.x)));
					}
				}
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -0.0 * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(poiFragData.baseColor + 0.0 * hueShiftAlpha.g);
				#endif
