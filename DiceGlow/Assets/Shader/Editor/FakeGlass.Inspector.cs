	...
	public partial class FakeGlassInspector : ShaderGUI
	{
		public class SCFBoot : AssetPostprocessor {
			...
			var isUpdated = importedAssets.Any(path => path.StartsWith("Assets/")) &&
							importedAssets.Any(path => path.Contains("SCF_InspectorData"));
			...
			FakeGlassInspector.LoadInspectorData();
			}
		}
		...
		public static void LoadInspectorData()
		{
			...
			//if (styles.Count == 0)
			{
					string[] guids = AssetDatabase.FindAssets("t:TextAsset SCF_InspectorData." + Application.systemLanguage);
					if (guids.Length == 0)
					{
						guids = AssetDatabase.FindAssets("t:TextAsset SCF_InspectorData.English");
