Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/6a0779b6826ed978bb65dc50cdf8b296"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			{
				float MainColorFeatherStep = float(0.583) - float(0.4);
				float firstColorFeatherStep = float(0.126) - float(0.305);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				firstShadeMap.rgb *= float4(0.2140411,0.2140411,0.1634667,1).rgb; //* lighColor
				secondShadeMap.rgb *= float4(0.03550518,0.03511393,0.02919445,1).rgb; //* LightColor;
				float shadowMask = 1;
				...
				float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.583) - MainColorFeatherStep) * (shadowMask));
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / (float(0.126) - firstColorFeatherStep) * (shadowMask));
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float3 directColor = _LightColor0.rgb * saturate(RTWrapFunc(poiLight.nDotL, float(0.571), float(0))) * lerp(1, poiLight.attenuation, poiLight.attenuationStrength);
				float3 indirectColor = ShadeSH9_wrapped(poiMesh.normals[float(0)], float(0.571)) * poiLight.occlusion;
				...
				poiLight.rampedLightMap = saturate((poiLight.lightMap - (1 - float(0.548))) / saturate((1 - float(0)) - (1 - float(0.548)) + fwidth(poiLight.lightMap)));
				...
				if (float(1) == 1)
				{
					...
					#endif
					if (float(0.636) == float(0)) float(0.636) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0.636), float(0), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.636), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.636), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(1) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.636), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				clip(poiFragData.alpha - float(0.956));
