...
namespace Voy.IntermediateAvatar.Utils
{
    ...
    {
        public static void Merge(AnimatorController[] srcs, AnimatorController dest, AnimatorType type = AnimatorType.FX)
        {
            if (srcs is null) throw new InvalidOperationException($"{nameof(srcs)} is null");
            if (dest is null) throw new InvalidOperationException($"{nameof(dest)} is null");

            ClearAnimatorController(dest);

            foreach (var src in srcs)
            {
                if (src is null) continue;
                AppendAnimatorController(src, dest, type);
            }
        ...
        private static void AppendAnimatorController(AnimatorController src, AnimatorController dest, AnimatorType type = AnimatorType.FX)
        {
            ...
            {
                AnimatorControllerUtility.AddLayer(dest, layer, type, firstLayer, destAssetPath);
                firstLayer = false;
            }
        }
        ...
        private static void CombineAnimatorController(AnimatorController srcController, AnimatorController dstController, AnimatorType type)
        {
            var dstControllerPath = AssetDatabase.GetAssetPath(dstController);

            for (int i = 0; i < srcController.layers.Length; i++)
            {
                AddLayer(dstController, srcController.layers[i], type, i == 0, dstControllerPath);
            }
        ...
        private static AnimatorControllerLayer AddLayer(AnimatorController controller, AnimatorControllerLayer srcLayer, AnimatorType type = AnimatorType.FX, bool setWeightTo1 = false, string controllerPath = "")
        {
            ...
            var newLayer = DuplicateLayer(srcLayer, controller.MakeUniqueLayerName(srcLayer.name), controllerPath, type, setWeightTo1);
            controller.AddLayer(newLayer);

            newLayer.name = "[" + type.ToString() + "] ";
        ...
        private static AnimatorControllerParameter AddParameter(AnimatorController controller, AnimatorControllerParameter srcParameter)
        {
        ...
        private static AnimatorControllerLayer DuplicateLayer(AnimatorControllerLayer srcLayer, string dstLayerName, string controllerPath, AnimatorType type, bool firstLayer = false)
        {
        ...
        private static AnimatorController DuplicateAnimationLayerController(string originalControllerPath, string outputFolderPath, string avatarName)
        {
