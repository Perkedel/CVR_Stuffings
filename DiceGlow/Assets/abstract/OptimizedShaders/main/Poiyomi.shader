Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/d20c65041b06cda49a1bfe8612c2a540"
{
	Properties
	{
 ...
 #define PROP_MAINCOLORADJUSTTEXTURE 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(0))
				{
					...
					if (float(0.5) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(90), float(0.5));
					}
					else
					{
					...
					float brightness = sin((_Time.x + offset) * float(0.5)) * float(20) - (float(20) - 1);
				...
				float glitterMask = 1;
				#endif
				glitterMask *= lerp(1, poiLight.rampedLightMap, float(0.99));
				...
				if (float(1) == 1)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * float(8.7), finalGlitter * glitterTexture.a * glitterMask);
					poiFragData.emission += finalGlitter * glitterColor * max(0, (float(8.7) - 1) * glitterTexture.a) * glitterMask;
				}
				else
				{
					poiFragData.emission += finalGlitter * glitterColor * float(8.7) * glitterTexture.a * glitterMask;
				}
			}
			#endif
			//ifex float(1)==0 && float(0)==0
			...
			{
				float smoothness = float(0.406);
				float smoothness2 = float(1);
				float metallic = float(0.194);
