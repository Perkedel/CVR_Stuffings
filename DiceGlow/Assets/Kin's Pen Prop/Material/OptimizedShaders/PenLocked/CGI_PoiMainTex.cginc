...
void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
{
    ...
    mainTexture.a *= alphaMask;
        ...
        #ifdef FINALPASS
            #if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
                half3 detailTexture = POI2D_SAMPLER_PAN(_DetailTex, _MainTex, poiMesh.uv[(0.0 /*_DetailTexUV*/)], float4(0,0,0,0)).rgb * float4(1,1,1,1).rgb;
            #else
                half3 detailTexture = 0.21763764082 * float4(1,1,1,1).rgb;
            #endif
            albedo.rgb *= LerpWhiteTo(detailTexture * (1.0 /*_DetailBrightness*/) * unity_ColorSpaceDouble.rgb, detailMask.r * (1.0 /*_DetailTexIntensity*/) * backFaceDetailIntensity);
        #endif
