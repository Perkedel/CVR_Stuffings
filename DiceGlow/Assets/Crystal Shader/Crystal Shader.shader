    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float4, _BasaColor)
...
////// Emissive:
                ...
                float _ChromaticAberration_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ChromaticAberration );
                float4 _BasaColor_var = UNITY_ACCESS_INSTANCED_PROP( Props, _BasaColor );
                float4 _BaseColortex_var = tex2D(_BaseColortex,TRANSFORM_TEX(i.uv0, _BaseColortex));
                float3 emissive = saturate(((lerp(((saturate(3.0*abs(1.0-2.0*frac(((0.5+node_1156)*_ColorLoop_var)+float3(0.0,-1.0/3.0,1.0/3.0)))-1)*node_8465)+(node_5582*saturate(3.0*abs(1.0-2.0*frac((node_7214*_ColorLoop_var)+float3(0.0,-1.0/3.0,1.0/3.0)))-1))),float3(node_3443,node_3443,node_3443),_PasutelColor_var)*_ColoeLevel_var*_LightCompletion_var)+(float3(tex2D( _GrabTexture, ((node_4873*pow(1.0-max(0,dot(normalDirection, viewDirection)),(_Distortion_var-_ChromaticAberration_var)))+sceneUVs.rg)).r,tex2D( _GrabTexture, ((node_4873*pow(1.0-max(0,dot(normalDirection, viewDirection)),_Distortion_var))+sceneUVs.rg)).g,tex2D( _GrabTexture, ((node_4873*pow(1.0-max(0,dot(normalDirection, viewDirection)),(_Distortion_var+_ChromaticAberration_var)))+sceneUVs.rg)).b)*_BasaColor_var.rgb*_BaseColortex_var.rgb)));
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float4 _BasaColor_var = UNITY_ACCESS_INSTANCED_PROP( Props, _BasaColor );
                float4 _BaseColortex_var = tex2D(_BaseColortex,TRANSFORM_TEX(i.uv0, _BaseColortex));
                o.Emission = saturate(((lerp(((saturate(3.0*abs(1.0-2.0*frac(((0.5+node_1156)*_ColorLoop_var)+float3(0.0,-1.0/3.0,1.0/3.0)))-1)*node_8465)+(node_5582*saturate(3.0*abs(1.0-2.0*frac((node_7214*_ColorLoop_var)+float3(0.0,-1.0/3.0,1.0/3.0)))-1))),float3(node_3443,node_3443,node_3443),_PasutelColor_var)*_ColoeLevel_var*_LightCompletion_var)+(float3(tex2D( _GrabTexture, ((node_4873*pow(1.0-max(0,dot(normalDirection, viewDirection)),(_Distortion_var-_ChromaticAberration_var)))+sceneUVs.rg)).r,tex2D( _GrabTexture, ((node_4873*pow(1.0-max(0,dot(normalDirection, viewDirection)),_Distortion_var))+sceneUVs.rg)).g,tex2D( _GrabTexture, ((node_4873*pow(1.0-max(0,dot(normalDirection, viewDirection)),(_Distortion_var+_ChromaticAberration_var)))+sceneUVs.rg)).b)*_BasaColor_var.rgb*_BaseColortex_var.rgb)));
