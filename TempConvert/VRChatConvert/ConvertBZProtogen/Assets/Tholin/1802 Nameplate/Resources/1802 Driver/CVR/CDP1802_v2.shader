Shader "Tholin/CDP1802_v2_cached_CVR"
{
	Properties
	{
		...
		[Toggle] _Reset ("Reset", Int) = 1
		[Toggle] _Pause ("Pause", Int) = 0
		_Input ("Input", Int) = 0
	}
	SubShader
	{
		...
		Pass
		{
			...
			uint4 frag (v2f i) : COLOR
			{
				...
				//Which bit are we on?
				uint idxx = (uint)(i.uv.x * 256.0);
				uint idxy = (uint)(i.uv.y * 256.0);
				uint idx = idxy * 256 + idxx;

				if(status == 0 || _Reset) { //Needs to be initialized
